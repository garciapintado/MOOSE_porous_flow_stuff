# mpiexec -np 4 ../../archaea-opt -i syn09.i
# problem as 08.i but making the mantle not subject to Darcy flow.
# this implies pressure, Darcy velocities, flow Material and Kernels are restricted to the upper blocks.
...
[Kernels] # as added by the action [PorousFlowFullySaturated]
  ...
  [heat_time_derivative_nopore]
    type = PorousFlowEnergyTimeDerivativeNopore      # this kernel lumps the heat energy-density to the nodes to ensure superior numerical stabilization
    block = 'mantle'
    variable = temperature
    neglect_porosity = true
  []
  [heat_conduction]
...
[Materials]
  ...
  [lambda_unsat]
    ...
    lambda_s = '2.3 0 0 0 2.3 0 0 0 2.3'
  []
  # materials added by PorousFlowFullySaturated Action
  ...
  [temperature_material]
    ...
    #block =                              # I think I would not need this here, as temperature exists in all blocks 
  ...
  [simple_fluid_noflow]
    ...
    fp = the_simple_fluid                      # this Material is at_nodes=false by default
    #compute = false
    #compute_density_and_viscosity = false
