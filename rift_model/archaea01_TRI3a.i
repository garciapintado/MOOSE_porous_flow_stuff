# run as:
# archaea-opt -i archaea01_TRI3a.i --mesh-only
#
# mpiexec -np 4 archaea-opt -i archaea01_TRI3s.i # Num Local DOFs: 5380
# [output: archaea01_TRI3a_exodus.e]
#
# +++ purpose +++
# Test: domain inherited from rift2ridge2
# - 'unsat' and 'nopore' subdomains: only thermal conduction
# - remaining domains: PorousFlow unsaturated TH simulation with advection + conduction
#
# this test aims to evaluate the use of an top internal boundary condition for porepressure, 
# underlying the 'unsat' zone
#  
# {mpiexec -np 4, simple_fluid, 10 kyr run} => 11.2 minutes in my iMAC
# {mpiexec -np 4, tabulate EOS, 10 kyr run} => early crash after a few converging steps
# The simple fluid run shows a noisy pattern in the temperature field close to 'unsat' but
# Darcy velocities close to the ocean bottom seem to make sense
...
[UserObjects]
  ...
  [pc] # // Add the capillary pressure UserObject
    ...
    block = 'mantle lcrust ucrust sediment'
    alpha = 1E-6
    m = 0.6
  []
  [pp_KT_advectiveflux_onecomp_userobj]
    type = PorousFlowAdvectiveFluxCalculatorUnsaturated       # [1-phase, 1-comp, fully saturated] one kernel for each fluid component
    block = 'mantle lcrust ucrust sediment'
    phase = 0
...
[AuxVariables]
  ...
  []
   [mass_flux_top_submarine_source]
    ...
    order = FIRST
  []
  [mass_flux_top_saturated_sink]
    block = 'mantle lcrust ucrust sediment'
    family = LAGRANGE
    order = FIRST
  []
  [mass_flux_top_saturated_source]
    block = 'mantle lcrust ucrust sediment'
    family = LAGRANGE
    order = FIRST
  []
  ...
  [heat_flux_adv_top_submarine_source]
    ...
    order = FIRST
  []
  #[heat_flux_adv_top_saturated_sink]
  #  block = 'mantle lcrust ucrust sediment'
  #  family = LAGRANGE
  #  order = FIRST
  #[]
  #[heat_flux_adv_top_saturated_source]
  #  block = 'mantle lcrust ucrust sediment'
  #  family = LAGRANGE
  #  order = FIRST
  #[]
...
[Kernels] # as added by the action [PorousFlowFullySaturated]
  ...
  []
  [heat_time_derivative_unsat]
    type = SpecificHeatConductionTimeDerivative
    block = 'unsat'
    variable = temperature
    #lumping = true | lumping here prevents convergence! Don't know why
    density = density_unsat
    specific_heat = specific_heat_unsat
  []
  [heat_conduction_unsat]
    type = HeatConduction
    block = 'unsat'
    variable = temperature
    diffusion_coefficient = thermal_conductivity_unsat
  []
[]
...
[BCs]
  ...
  [Ptop_saturated_sink]
    type = PorousFlowPiecewiseLinearSink
    variable = porepressure
    boundary = top_saturated               # nodeset top-bounding the subDomain for porous flow simulation & not touching the ocean bottom
    PT_shift = ptop_auxvar
    pt_vals = '0 1e9'
    multipliers = '0 1e9'
    use_mobility = true
    ...
    flux_function = 0.01
    fluid_phase = 0
    save_in = mass_flux_top_saturated_sink
  []
  [Ptop_saturated_source]
    type = PorousFlowPiecewiseLinearSink
    variable = porepressure
    boundary = top_saturated
    PT_shift = ptop_auxvar
    pt_vals = '-1e9 0'
    multipliers = '-1e9 0'
    use_mobility = true
    ...
    flux_function = 0.1
    fluid_phase = 0
    save_in = mass_flux_top_saturated_source
  []
  [Ttop_submarine_adv_sink] # advective heat flux = enthalpy * darcy Flux
    type = PorousFlowPiecewiseLinearSink
    variable = temperature
    boundary = top_submarine
    PT_shift = ptop_auxvar
  ...
  []
  # Could the following two BC blocks stabilise the temperature field around the 'unsat' area?
  # Do they make physical sense considering that no mechanical coupling plays here and
  # that the extra flow that may come from 'unsat' is sort of a compensation for the lack of mechanical?  
  #[Ttop_saturated_adv_sink]
  #  type = PorousFlowPiecewiseLinearSink
  #  variable = temperature
  #  boundary = top_saturated
  #  PT_shift = ptop_auxvar
  ...
  #  flux_function = 0.01
  #  fluid_phase = 0
  #  save_in = heat_flux_adv_top_saturated_sink
  #[]
  #[Ttop_saturated_adv_source] 
  #  type = PorousFlowPiecewiseLinearSink  # 
  #  variable = temperature
  #  boundary = top_saturated
  #  PT_shift = ptop_auxvar
  ...
  #  flux_function = 0.1
  #  fluid_phase = 0
  #  save_in = heat_flux_adv_top_saturated_source
  #[]
  [Ttop_submarine_diff] # diffusive heat flux = - lambda * grad_T
    type = PorousFlowPiecewiseLinearSink
    variable = temperature
  ...
  []
  [Ttop_subaerial_diff] # diffusive heat flux
    type = DirichletBC 
    variable = temperature
...
[Materials]
  ...
  [temperature_material]
    ...
    block = 'mantle lcrust ucrust sediment' # despite the variable temperature exists in all blocks, this material only applies to the PorousFlow subdomain
  ...
  [nearest_qp]                                 # according to the example multiblock.i, this seems not needed
    ...
    block = 'mantle lcrust ucrust sediment'
  []
  # materials not explicitly added by the Action PorousFlowFullySaturated:
  [porosity]
...
[Executioner]
  ...
  #end_time = 31557600000      # [s] 1000 yr
  end_time =  315576000000     # 10 kyr
