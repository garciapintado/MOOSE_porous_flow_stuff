...
#include "PorousFlowSingleComponentFluidTruncated.h"

registerMooseObject("PorousFlowApp", PorousFlowSingleComponentFluidTruncated);

InputParameters
PorousFlowSingleComponentFluidTruncated::validParams()
{
  ...
  params.addParam<Real>("minimum_temperature",273.2,"minimum input temperature [K] used for the FluidProperties"
                       "module. Values lower than this will be truncated to this value" );
  params.addParam<Real>("maximum_temperature",1300,"maximum input temperature [K] used for the FluidProperties"
                       "module. Values higher than this will be truncated to this value" );
...
PorousFlowSingleComponentFluidTruncated::PorousFlowSingleComponentFluidTruncated(const InputParameters & parameters)
  : PorousFlowFluidPropertiesBase(parameters),
    ...
    _min_pressure(getParam<Real>("minimum_pressure")),
    _min_temperature(getParam<Real>("minimum_temperature")),
    _max_temperature(getParam<Real>("maximum_temperature")),
...
void
PorousFlowSingleComponentFluidTruncated::initQpStatefulProperties()
{
  const Real Pp = std::max(_porepressure[_qp][_phase_num] * _pressure_to_Pascals, _min_pressure); 
  const Real Tk = std::min(std::max(_temperature[_qp] + _t_c2k, _min_temperature), _max_temperature);
  if (_compute_rho_mu)
    (*_density)[_qp] = _fp.rho_from_p_T(Pp, Tk);
  if (_compute_internal_energy)
    (*_internal_energy)[_qp] = _fp.e_from_p_T(Pp, Tk);
  if (_compute_enthalpy)
    (*_enthalpy)[_qp] = _fp.h_from_p_T(Pp, Tk);
}

void
PorousFlowSingleComponentFluidTruncated::computeQpProperties()
{
  const Real Pp = std::max(_porepressure[_qp][_phase_num] * _pressure_to_Pascals, _min_pressure); 
  const Real Tk = std::min(std::max(_temperature[_qp] + _t_c2k, _min_temperature), _max_temperature);
  if (_compute_rho_mu)
  ...
  {
    Real e, de_dp, de_dT;
    _fp.e_from_p_T(Pp, Tk, e, de_dp, de_dT);
  ...
  {
    Real h, dh_dp, dh_dT;
    _fp.h_from_p_T(Pp, Tk, h, dh_dp, dh_dT);
    (*_enthalpy)[_qp] = h;
